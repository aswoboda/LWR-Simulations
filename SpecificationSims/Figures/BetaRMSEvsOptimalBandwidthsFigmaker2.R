...
# 
pdf("SpecificationSims/Figures/BetaRMSEvsOptimalBandwidths2.pdf")
par(mfcol = c(3, 3))
  ...
  for (my.B1 in 1:3) {
    for (my.beta in c("B0", "B1", "B2")) {
      for (my.error in 1:3) {
        ...
        axis(1)
        axis(2)
        if(my.error == 1) mtext(my.beta, side = 3, cex = .8, col = "orange")
        if(my.beta == "B0") mtext(paste("error sd=", dimnames(MetricOutput)$error.sd[my.error]), side = 2, line = 2.5, , cex = .8, col = "orange")
