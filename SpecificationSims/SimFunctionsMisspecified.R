...
LWR = function( my.observation, Data.Frame, my.model = "dep.var ~ indep.var1") {
  #my.observation = 2
  #Data.Frame = test.data
  #my.model = "dep.var ~ indep.var1"
...
Reorganizer = function(lapplyoutput) {
  ...
  temp1 = unlist(temp)
  beta0hats = matrix(temp1[seq(1, length(temp1), 2)], n, ks, byrow = T)
  beta1hats = matrix(temp1[seq(2, length(temp1), 2)], n, ks, byrow = T)
  #beta2hats = matrix(temp1[seq(3, length(temp1), 3)], n, ks, byrow = T)
  
  # standard errors
  temp = sapply(lapplyoutput, "[", 2) # grabbing the estimated beta values
  temp1 = unlist(temp)
  ses0 = matrix(temp1[seq(1, length(temp1), 2)], n, ks, byrow = T)
  ses1 = matrix(temp1[seq(2, length(temp1), 2)], n, ks, byrow = T)
...
RsquaredComparer = function(Data) {
  ...
  ols = lm(dep.var ~ indep.var1, data = Data)
  ols.sum = summary(ols)
