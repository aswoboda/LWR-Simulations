...
<<SpatVarBeta, cache=TRUE, fig = T, fig.height= 4, fig.width=7>>=
east = north = 0:10
...
for (mymetric in mymetrics) {
  summary.table[which(mymetrics == mymetric), 1:6] = summary(MetricOutput[ , , , , , mymetric, "bandwidths"])
  summary.table[which(mymetrics == mymetric), 7] = sd(MetricOutput[ , , , , , mymetric, "bandwidths"])
}
...
for (ssi in myss) {
  summary.table = matrix(0, length(mymetrics), 7)
  for (mymetric in mymetrics) {
    summary.table[which(mymetrics == mymetric), 1:6] = summary(MetricOutput[ ssi, , , , , mymetric, "bandwidths"])
    summary.table[which(mymetrics == mymetric), 7] = sd(MetricOutput[ ssi, , , , , mymetric, "bandwidths"])
  }
  rownames(summary.table) = mymetrics
  colnames(summary.table) = c("min", "Q1", "median", "mean", "Q3", "max", "sd")
...
<<beanplots, dev='tikz', cache=TRUE, fig.height = 8, fig.width = 6, fig = T, tidy= FALSE, external=TRUE, fig.cap= 'Hello World. This is a figure caption. I am going to keep writing for a bit to see what happens if I just keep writing and writing. What is the point of this graphic? You will find that out right here.'>>=
require(beanplot)
...
<<MegaFig, dev='tikz', cache=TRUE, fig.height= 7, fig.width=7, fig.align='left', out.width='.9\\textwidth'>>=
