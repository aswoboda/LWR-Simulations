...
uberFunction <- function(repetition, sampleSize, errorSD, B0.SpVar, B1.SpVar, B2.SpVar){
  ...
  if(B0.SpVar == 0){
    ...
    #there is slight correlation between B0 and B1,B2, but it is smaller between B0 and B1*x1 and B2*x2
...
#  x <- uberFunction(1, sampleSize = 29, 1.5, 0, 1, 0)
#  x
...
mcMultParams <- function(dataGenParameters, MC = FALSE){ #runs data on a list of true models for a list of sample sizes and error terms
  ...
  write.csv(uberListMatrix, file = paste0("Uber Output Large Test.csv"))
  uberListMatrix
}
