...
Thus far two models have been discussed - the standard OLS model in which all of the coefficients are stationary and a local model in which all coefficients are estimated locally using some bandwidth. The two cases are identical in the extreme case of an infinite bandwidth. Other ``mixed'' models are also possible in which some coefficients are assumed to be stationary while others are estimated locally. In the example described in Equation \eqref{eq:simpleOLS} with three coefficients , there are are eight ($2^3$) total models, ranging from the LWR model in which all three coefficients are assumed to be ``Local'', three models in which two coefficients are ``Local'' and one is assumed to be ``Global'', three models in which one coefficient is ``Local'' and the remaining two coefficients are ``Global'' and the OLS model in which all three coefficients are ``Global''.
...
pdf("figure/SpatialVariationBetas.pdf", width = 8, height = 6)
...
persp(east, north, B1.0, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")
B1.1 = outer(east, north, B1func, B1.SpVar = 2)
persp(east, north, B1.1, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")
B1.2 = outer(east, north, B1func, B1.SpVar = 4)
persp(east, north, B1.2, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")

B2.0 = outer(east, north, B2func, B2.SpVar = 0)
persp(east, north, B2.0, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")
B2.1 = outer(east, north, B2func, B2.SpVar = 2)
persp(east, north, B2.1, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")
B2.2 = outer(east, north, B2func, B2.SpVar = 4)
persp(east, north, B2.2, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")

B0.0 = outer(east, north, B0func, B0.SpVar = 0)
persp(east, north, B0.0, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")
B0.1 = outer(east, north, B0func, B0.SpVar = 2)
persp(east, north, B0.1, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")
B0.2 = outer(east, north, B0func, B0.SpVar = 4)
persp(east, north, B0.2, zlim = c(-0.6, 7.6), theta = 30, phi = 30, zlab = "")

mtext(expression(paste(eta, " = 0")), at = .15, side = 3, outer = TRUE, cex = 1.5)
mtext(expression(paste(eta, " = 2")), at = .48, side = 3, outer = TRUE, cex = 1.5)
mtext(expression(paste(eta, " = 4")), at = .81, side = 3, outer = TRUE, cex = 1.5)
...
\makebox[\textwidth][c]{\includegraphics[width = 1.1\textwidth]{figure/SpatialVariationBetas}}
...
No one in our sample uses SCV, but do a google scholar search and look for articles that cite the original articles
