...
<<ModelSelectionTabulations, echo = FALSE>>=
for (i in 1:1) {
...
<<BandwidthsLGG, echo = FALSE, results='hide'>>=
  TrueModel1 = which(mcOutput[1, "True Model", ] == 2)
pdf("figure/ModelAndBandwidthByMetric-LGG.pdf", width = 8, height = 10)
par(mar = c(0.5, 6, 6, 0.5))
par(xpd = NA)
par(mfrow = c(4, 2))
  for (mymetric in dimnames(mcOutput)[[1]][8:14]) {
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics[width = 1.2\textwidth]{figure/ModelAndBandwidthByMetric-LGG.pdf}}
\caption{This figure shows }
\label{fig:LGGmodelBandwidths}
\end{figure}

Figure \ref{fig:LGGmodelBandwidths} shows

<<BandwidthsError, echo = FALSE, results='hide'>>=
pdf("figure/ModelAndBandwidthByMetric.pdf", width = 20, height = 18)
par(mar = c(0.5, 6, 6, 0.5))
par(xpd = NA)
par(mfcol = c(7, 5))
for (mymodel in 2:8) {
...
\begin{figure}
  \makebox[\textwidth][c]{\includegraphics[width = 1.2\textwidth]{figure/ModelAndBandwidthByMetric-GLG.pdf}}
\caption{This figure shows }
