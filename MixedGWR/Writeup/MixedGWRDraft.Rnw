...
% \url{https://scholar.google.com/scholar?cites=11649932945261630952&as_sdt=5,24&sciodt=0,24&hl=en}
% 
% 
...
% 
% \url{https://scholar.google.com/scholar?as_ylo=2011&hl=en&as_sdt=5,24&sciodt=0,24&cites=9654495652239805234&scipsc=}
...
layout(matrix(c(1,1,2,2,3,3,4,4, 8,5,5,6,6,7,7,9), 2, 8, byrow = TRUE))
  for (mymetric in dimnames(mcOutput)[[1]][c(11, 9, 10, 8, 12, 13, 14)]) {
    ...
    tempdata = data.frame(ModelNum = factor(output[1, ], levels = 1:8),
      ...
      mtext("smallest ---- bandwidth size ----  biggest", 3, line = 0.5, cex = .7)
  }
#dev.off()
@
...
TrueModel1 = which(mcOutput[1, "B0 SpVar", ] == SpVar & 
  ...
  for (mymetric in dimnames(mcOutput)[[1]][c(11, 9, 10, 8)]) {
    ...
    tempdata = data.frame(ModelNum = factor(output[1, ], levels = 1:8),
      ...
      #mtext(paste("metric: ", mymetric), 3, line = 2.5, cex = 1.25)
      mtext("small --- bandwidth size ---  big", 3, line = 0.5, cex = .6)
  }
}
...
mtext(c("none", "some", "more"), side = 3, outer = TRUE,
      at = c(.21, .54, .88))
mtext("Spatial Variation in All Three Coefficients:", side = 3, outer = TRUE, at = .52, line = 2, cex = 1.2)
