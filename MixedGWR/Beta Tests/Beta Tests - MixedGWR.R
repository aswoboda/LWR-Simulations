...
sampleSizes <- c(300, 500, 700)

trueModelNumber <- 2

numRepeats = 3 # the number of times we'll run this simulation

numk = 7 # number of bandwidths we'll use
krat = 2/3 # rate at which bandwidths decrease (ie. 45 -> 30 -> 20, etc.)

# naming functions
plotTitle <- function(trueModelName, sampleSize){
  title <- paste0("Sample Size for ", trueModelName, " of ", sampleSize, " Observations for Beta Metric Testing")
  title
}
csvTitle <- function(){
  "Beta Test Results (GCV Model Comparision).csv"
}

BresultsTitle <- function(){
  "Beta Test Results (Coefficient Comparision).csv"
}

pdfTitle <- function(sampleSize, trueModelName){
  paste0("Sample size of ", sampleSize, " for ",trueModelName, " with B0 = 10(north + east), B1 = B2 = 10.pdf")
}
...
results <- matrix(data = NA, nrow = length(sampleSizes)*numRepeats, ncol = 13, 
                  dimnames = list(1:(length(sampleSizes)*numRepeats), c("True Model Number", "Model Error", "Sample Size", "Repetition", 
...
maxSampleSize <- max(sampleSizes)
numSampleSizes <- length(sampleSizes)
#can make the name for repetition more informative later, would require some integer division

Bresults <- array(NA, dim = c(maxSampleSize, 6, numk, numRepeats, 8, numSampleSizes), 
                  dimnames = list(c(paste("Observation", 1:maxSampleSize)), 
                                  c("True B0", "True B1", "True B2", "Estimated B0", "Estimated B1", "Estimated B2"), 
                                  c(paste("Bandwidth", 1:numk)),
                                  c(paste("Repetition", 1:numRepeats)),
                                  c(paste("Assumed Model", 1:8)),
                                  c(paste("Sample Size of", sampleSizes))))


trialNumber <- 0
#for(trueModelNumber in 5:8) {
  for(sampleSize in sampleSizes){
    ...
    pdf(pdfTitle(sampleSize, trueModelName), height = 8, width = 10)
    
    numRepetitions <- 0 
    for (repeats in 1:numRepeats) { 
      numRepetitions <- numRepetitions +1 #this is for the beta results, they need to reset after each sample size
      trialNumber <- trialNumber + 1 #this is for the broad results, it does not
      start <- Sys.time()
      ####
      ...
      if(trueModelNumber == 1){
        ...
        B1 = 10 # but this one is not
        B2 = 10 # nor is this coefficient
      ...
      for(assumedModelNum in 1:8){
        for(bandwidthNum in 1:numk){
          Bresults[paste("Observation", 1:n), "True B0", paste("Bandwidth", bandwidthNum), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", assumedModelNum), paste0("Sample Size of ", n)] <- B0
          Bresults[paste("Observation", 1:n), "True B1", paste("Bandwidth", bandwidthNum), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", assumedModelNum), paste0("Sample Size of ", n)] <- B1      
          Bresults[paste("Observation", 1:n), "True B2", paste("Bandwidth", bandwidthNum), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", assumedModelNum), paste0("Sample Size of ", n)] <- B2
        } 
      }
      ...
      ###Beta edits, only uses global bandwidth
      Bresults[paste("Observation", 1:n), "Estimated B0", paste("Bandwidth", numk), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", 1), paste0("Sample Size of ", n)] <- lmOLS$coeff[1]
      Bresults[paste("Observation", 1:n), "Estimated B1", paste("Bandwidth", numk), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", 1), paste0("Sample Size of ", n)] <- lmOLS$coeff[2]      
      Bresults[paste("Observation", 1:n), "Estimated B2", paste("Bandwidth", numk), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", 1), paste0("Sample Size of ", n)] <- lmOLS$coeff[3]
      ...
      for (modelnum in 2:7) { # these are the models that include a combination of stationary and non-stationary coefficients
        ...
        for (j in 1:numk) { # this loop goes through all of the different bandwidth values
          ...
          Bresults[paste("Observation", 1:n), "Estimated B0", paste("Bandwidth", j), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", modelnum), paste0("Sample Size of ", n)] <- betas[,"x0"]
          Bresults[paste("Observation", 1:n), "Estimated B1", paste("Bandwidth", j), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", modelnum), paste0("Sample Size of ", n)] <- betas[,"x1"]   
          Bresults[paste("Observation", 1:n), "Estimated B2", paste("Bandwidth", j), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", modelnum), paste0("Sample Size of ", n)] <- betas[,"x2"]
      ...
      for (j in 1:numk) { # for each k value
        ...
        for (i in 1:n) { # now for each observation
          ...
          #beta results
          Bresults[paste("Observation", i), "Estimated B0", paste("Bandwidth", j), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", 8), paste0("Sample Size of ", n)] <- lmreg$coeff[1]
          Bresults[paste("Observation", i), "Estimated B1", paste("Bandwidth", j), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", 8), paste0("Sample Size of ", n)] <- lmreg$coeff[2]
          Bresults[paste("Observation", i), "Estimated B2", paste("Bandwidth", j), paste0("Repetition ", numRepetitions), paste0("Assumed Model ", 8), paste0("Sample Size of ", n)] <- lmreg$coeff[3]
      ...
      matplot(GCVmat[, 9], GCVmat[, -9], type = "o", # this makes the plot that goes in the .pdf file
              ...
              ylab = "GCV score",
              main = plotTitle(trueModelName, sampleSize))
      ...
      #determine if the best model is the true model
      ...
      #to prevent a low score at two bandwidths giving the best model first and second, I remove the best model and the bandwidths
      
      GCVmatSecondBest <- GCVmat[,c(-results[resultsNumber, "Best Model Number"], -9)]
      
      #ranks GCV scores and picks out second lowest
      rankedGCV <- sort(GCVmatSecondBest)    
      ...
      print(paste0("This repetition of ", sampleSize, " observations on model ", trueModelNumber, " with error of ", round(errorSD, 2), " took ", round(difftime(end, start, units = "m"), 2), " min."))
    } #end of one repeat
...
write.csv(results, file = csvTitle()) #write csv with the results

#interpreting the beta results
BresultsMetrics <- BresultsMetrics.gen(Bresults, sampleSizes)

#to correctly find minimums, I replace all zeros with NAs
BresultsMetrics[BresultsMetrics == 0] <- NA


bestCoeff <- matrix(NA, nrow = 3*3*length(sampleSizes)*numRepeats, ncol = 7)
colnames(bestCoeff) <- c("Metric", "Beta", "Bandwidth", "Repetition", "Assumed Model", "Sample Size", "Value")


rownum <- 1
for(ss in 1:length(sampleSizes)){
  for(repeatnum in 1:numRepeats){
    
    #best B0 by the three metrics
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[1,"B0",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T), BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[1,"B0",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[2,"B0",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T), BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[2,"B0",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[3,"B0",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T)[1,], BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[3,"B0",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1

    #best B1 by the three metrics
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[1,"B1",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T), BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[1,"B1",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[2,"B1",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T), BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[2,"B1",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[3,"B1",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T)[1,], BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[3,"B1",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1
    
    #best B2 by the three metrics
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[1,"B2",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T), BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[1,"B2",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[2,"B2",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T), BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[2,"B2",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1
    bestCoeff[rownum,1:6] <- get.dimNames(which(min(BresultsMetrics[3,"B2",,repeatnum,,ss], na.rm = T) == BresultsMetrics, arr.ind = T)[1,], BresultsMetrics)
    bestCoeff[rownum,7] <- min(BresultsMetrics[3,"B2",,repeatnum,,ss], na.rm = T)
    rownum <- rownum + 1
...
bestCoeff
colnames(bestCoeff) <- c("Metric", "Beta", "Bandwidth", "Repetition", "Assumed Model", "Sample Size")

write.csv(bestCoeff, file = BresultsTitle() )
