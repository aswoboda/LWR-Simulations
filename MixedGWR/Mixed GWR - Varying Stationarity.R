...
plotTitle <- function(trueModelName, sampleSize){
  title <- paste0("Sample Size for ", trueModelName, " of ", sampleSize, " Observations for Discontinous Intercept")
  title
}
csvTitle <- function(){
  "Discontinous Intercepts of 0, 2, 3, or 5.csv"
}

pdfTitle <- function(sampleSize, trueModelName){
  paste0("Sample size of ", sampleSize, " for ",trueModelName, " with B0 = 0, 2, 3, or 5")
}
...
results <- matrix(data = NA, nrow = length(sampleSizes)*numRepeats*4, ncol = 13, 
                  dimnames = list(1:(length(sampleSizes)*numRepeats*4), c("True Model Number", "Model Error", "Sample Size", "Repetition", 
...
#       results[resultsNumber, "Best Model Variables"] <- paste0(models[bestModelNum, 1], ", ", models[bestModelNum, 2], ", ", models[bestModelNum, 3])
      ...
      print(paste0("This repetition of ", sampleSize, " observations on model ", trueModelNumber, " with error of ", round(errorSD, 2), " took ", round(difftime(end, start, units = "m"), "min."))
    } #end of one repeat
...
BresultsRMSE <- BresultsRMSE.gen(Bresults, sampleSizes)

bestCoeff <- matrix(NA, nrow = 3, ncol = 4)
bestCoeff[1,] <- get.dimNames(which(min(BresultsRMSE[,"B0",,,,], na.rm = T) == BresultsRMSE, arr.ind = T), BresultsRMSE)[3:6]
bestCoeff[2,]<- get.dimNames(which(min(BresultsRMSE[,"B1",,,,], na.rm = T) == BresultsRMSE, arr.ind = T), BresultsRMSE)[3:6]
bestCoeff[3,] <- get.dimNames(which(min(BresultsRMSE[,"B2",,,,], na.rm = T) == BresultsRMSE, arr.ind = T), BresultsRMSE)[3:6]
bestCoeff
